...
const constants = require("../constants/shared")
...
const login = async (req, res) => {
    try {
        ...
        const token = jwtService.generateAccessToken(existedUser._id, false);
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const token = jwtService.generateAccessToken(existedUser._id, true);

        // Password resett link that is to be mailed to user 
        const link = `http://localhost:3000/user/reset_password/${existedUser._id}/${token}`;
